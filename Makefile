...
# my.domain/multiarch-operator-bundle:$VERSION and my.domain/multiarch-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= my.domain/multiarch-operator
...
docker-build: manifests generate ## Build docker image with the manager.
	$(ENGINE) build -t ${IMG} --platform=linux/${TARGETARCH} --build-arg BUILD_IMAGE=$(BUILD_IMAGE) --build-arg RUNTIME_IMAGE=$(RUNTIME_IMAGE) .
